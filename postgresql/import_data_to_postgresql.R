...
# Process House Votes
  ...
  head(Legislators)
  dbWriteTable(con, name='Legislators', value=Legislators, overwrite = TRUE);
...
# Process Legislature
  ...
  head(Legislature)
  dbWriteTable(con, name='Legislature', value=Legislature, overwrite = TRUE);
  
  
# Process Session
  Session <- read.csv(url('https://raw.githubusercontent.com/Data4Democracy/louisville-hackathon-mar2017/master/Session.csv'))
  head(Session)
  dbWriteTable(con, name='Session', value=Session, overwrite = TRUE);
